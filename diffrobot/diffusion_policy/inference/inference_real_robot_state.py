...
class RobotInferenceController:
    ...
    def setup_robot(self):
        ...
        self.panda.set_dynamic_rel(1.0, accel_rel=0.0005, jerk_rel=0.05)
    ...
    def load_transforms(self):
        with open(f'../runs/{self.saved_run_name}/transforms.json', 'r') as f:
            ...
            self.X_FE = np.array(trans['X_FE'])

        self.X_BO = self.estimate_marker_pose()

        if self.X_BO is None:
            print('Marker not found. Human intervention required...')
            pdb.set_trace()
    ...
    def estimate_marker_pose(self):
        ...
        X_BF = np.dot(X_BE, np.linalg.inv(self.X_FE))
        X_BC = np.dot(X_BF, self.X_FC)
    ...
    def get_obs(self):
        ...
        temp = self.marker_detector.estimate_pose()
        if temp is not None:
            self.X_BO = temp
