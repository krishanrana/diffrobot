        ...
        while True:
            ...
            for obj, data in self.estimators.items():
                ...
                if i == 0:
                    ...
                    pose = est.register(K=self.K, rgb=color, depth=depth, ob_mask=mask, iteration=data["est_refine_iter"])
                else:
                    pose = est.track_one(rgb=color, depth=depth, K=self.K, iteration=data["track_refine_iter"])

                center_pose = pose @ np.linalg.inv(to_origin)
                center_pose = center_pose @ np.linalg.inv(data["saved_transform"])
                self.pose_dict[obj] = center_pose

                # draw the xyz_axis for all objects on a single image then show it
